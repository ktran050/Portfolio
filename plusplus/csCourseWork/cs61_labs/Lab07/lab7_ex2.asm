...
; Set R4 as the switch with default true
LD R4, DEC_1
...
COMPARE_LOOP
; If switch == 1 then continue, else end
	ADD R6, R4, #-1
	BRnp END_COMPARE_LOOP
...
; If pointer == foil end
	...
	; ADD -R1 and R0
	ADD R6, R6, R0
	BRz END_COMPARE_LOOP

; Compare current pointer and its foil
	; Load both values
	LDR R3, R0, #0
	LDR R6, R1, #0
	; To subtract them turn one of them into a negative
	NOT R6, R6
	ADD R6, R6, #1
	; Now subtract
	ADD R6, R6, R3
