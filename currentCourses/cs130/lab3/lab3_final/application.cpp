...
GLuint loadShader(const char* vertexfilename, const char* fragmentfilename)
{
    ...
    // glShaderSource
    string str = getTextFile(vertexfilename);
    char *ptr = &str[0];
    int size = str.length();
    glShaderSource(vertex_shader_id, 1, &ptr, &size);

    str = getTextFile(fragmentfilename);
    ptr = &str[0];
    size = str.length();
    glShaderSource(fragment_shader_id,1, &ptr, &size);
