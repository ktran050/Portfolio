...
; Convert the return of menu (R1) from ASCII to DEC
LD R6, ASC_N_0
ADD R1, R1, R6

; If R1 is not 1 go to the next check
OP1
ADD R1, R1, #-1
BRnp OP2

; Else call ALL_MACHINES_BUSY
LD R6, ALL_MACHINES_BUSY
JSRR R6

; Output a response based on what the return of AMB
ADD R2, R2, #0
BRz AMB_FALSE
BRnp AMB_TRUE
...
; Else call ALL_MACHINES_FREE
LD R6, ALL_MACHINES_FREE
JSRR R6

; Output a response based on what the return of AMF
ADD R2, R2, #0
BRz AMF_FALSE
BRnp AMF_TRUE
...
; Else call NUM_BUSY_MACHINES
LD R6, NUM_BUSY_MACHINES
JSRR R6

; Output a response based on what the return of NBM
    ; Always output "There are "
    LEA R0, THERE_ARE
    TRAP x22
    
    ; If the number machines is 9 or below skip this part
    AND R6, R6, #0
    ADD R6, R6, #-10
    ADD R6, R6, R2
    BRn SKIP_NBM_10
    
    ; Else output a one and subtract 10
    LD R0, ASC_1
    TRAP x21
    ADD R2, R2, #-10
    
    ; Output whatever single number is left
    SKIP_NBM_10
    LD R6, ASC_0
    ADD R0, R2, R6
    TRAP x21
    
    ; Always output " busy machines\n"
    LEA R0, BUSY_MACH
    TRAP x22
; Short-circuit
BR MAIN
...
; Else call NUM_FREE_MACHINES
	; Call the subroutine
	LD R6, NUM_FREE_MACHINES
	JSRR R6
	
; Output based on the return (R2)
    ; Always output "There are "
    LEA R0, THERE_ARE
    TRAP x22
...
; Else call MACHINE_STATUS
LD R6, MACHINE_STATUS
JSRR R6

; Output a response based on what the return of MS
	; Always output "Machine "
	LEA R0, MACH_INE
	TRAP x22
	
	; Convert back for user
	AND R6, R6, #0
	ADD R6, R6, #15
	...
	; Output the machine we just checked
		; If the number machines is 9 or below skip this part
		AND R6, R6, #0
		ADD R6, R6, #-10
		ADD R6, R6, R3
	...
	; Check which output to go with
	ADD R2, R2, #0
	BRnp IZZ_FREE
	BRz IZZ_BIZ
	
	IZZ_FREE
	; If true output is  " free"
	LEA R0, IS_FREE
	TRAP x22
	; Short-circuit
	BR MAIN
...
; Else call FIRST_FREE
LD R6, FIRST_FREE
JSRR R6

; Output a response based on what the return of FF
	; If the return was not negative skip this
	ADD R2, R2, #0
	BRzp SKIP
	
	; Else output "No machines are free\n"
	LEA R0, NO_FREE
	TRAP x22
	BR MAIN
	...
	; Output which machine is free
	SKIP
	
			; Convert to account for 15 being most sig bit
			AND R6, R6, #0
			ADD R6, R6, #15
			NOT R2, R2
			ADD R2, R2, #1
			...
			; Always output "The first available machine is number "
			LEA R0, FIRST_AVAIL
			TRAP x22
...
; The only possibility left for R1 is 7, so output the quit message
OP7
LEA R0, RESPONSE7_1
TRAP x22
...
ADD R2, R2, #0
	; If there's no value then the enter is invalid
