...
int main(int argc, char *argv[]){
  ...
  int pid=0;
  pid=fork();
    1009:	e8 8c 02 00 00       	call   129a <fork>
  if(pid==0){
    100e:	85 c0                	test   %eax,%eax
    1010:	75 35                	jne    1047 <main+0x47>
    int x=1;
    printf(1,"address %x\n", &x);
    1012:	8d 44 24 1c          	lea    0x1c(%esp),%eax
    1016:	89 44 24 08          	mov    %eax,0x8(%esp)
    101a:	c7 44 24 04 a1 17 00 	movl   $0x17a1,0x4(%esp)
    1021:	00 
...
int main(int argc, char *argv[]){
  ...
  if(pid==0){
    ...
    1030:	00 
    printf(1,"address %x\n", &x);
    1031:	e8 ca 03 00 00       	call   1400 <printf>
#include "user.h"
...
test(n+1);
    ...
    103d:	e8 1e 00 00 00       	call   1060 <test>
  if(pid==0){
    int x=1;
    printf(1,"address %x\n", &x);
    test(1);
//    exec("null"m0);
    exit();
    1042:	e8 5b 02 00 00       	call   12a2 <exit>
  }
  wait();
    1047:	e8 5e 02 00 00       	call   12aa <wait>
#include "user.h"
...
//if(n<100)
test(n+1);
    104c:	c7 04 24 03 00 00 00 	movl   $0x3,(%esp)
    1053:	e8 08 00 00 00       	call   1060 <test>
//    exec("null"m0);
    exit();
  }
  wait();
...
int test(int n){
    ...
    1067:	8b 5d 08             	mov    0x8(%ebp),%ebx
//if(n<100)
//if(n<100)
test(n+1);
    106a:	8d 43 01             	lea    0x1(%ebx),%eax
    106d:	89 04 24             	mov    %eax,(%esp)
    1070:	e8 eb ff ff ff       	call   1060 <test>
return n;
}
    1075:	83 c4 14             	add    $0x14,%esp
    1078:	89 d8                	mov    %ebx,%eax
    107a:	5b                   	pop    %ebx
    107b:	5d                   	pop    %ebp
    107c:	c3                   	ret    
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    1399:	0f b6 92 b4 17 00 00 	movzbl 0x17b4(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    152f:	8b 38                	mov    (%eax),%edi
        ...
        if(s == 0)
          s = "(null)";
    1531:	b8 ad 17 00 00       	mov    $0x17ad,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15f1:	a1 90 1a 00 00       	mov    0x1a90,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    1639:	a3 90 1a 00 00       	mov    %eax,0x1a90
}
    163e:	5b                   	pop    %ebx
    163f:	5e                   	pop    %esi
    1640:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    1662:	a3 90 1a 00 00       	mov    %eax,0x1a90
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    168c:	8b 1d 90 1a 00 00    	mov    0x1a90,%ebx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
morecore(uint nu)
{
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16cb:	3b 15 90 1a 00 00    	cmp    0x1a90,%edx
...
morecore(uint nu)
{
  ...
  return freep;
    1702:	8b 15 90 1a 00 00    	mov    0x1a90,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    1724:	89 70 04             	mov    %esi,0x4(%eax)
      ...
      freep = prevp;
    1727:	89 15 90 1a 00 00    	mov    %edx,0x1a90
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    173e:	c7 05 90 1a 00 00 94 	movl   $0x1a94,0x1a90
    1745:	1a 00 00 
    base.s.size = 0;
    1748:	ba 94 1a 00 00       	mov    $0x1a94,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    174d:	c7 05 94 1a 00 00 94 	movl   $0x1a94,0x1a94
    1754:	1a 00 00 
    base.s.size = 0;
    1757:	c7 05 98 1a 00 00 00 	movl   $0x0,0x1a98
    175e:	00 00 00 
